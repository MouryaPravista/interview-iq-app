...
// Reusable UI components with the new theme
const PerformanceBar = ({ label, score }: { label: string; score: number }) => ( 
    <div> 
        ...
        <div className="w-full bg-gray-800 rounded-full h-2.5">
            <div className="bg-teal-500 h-2.5 rounded-full" style={{ width: `${Math.round(score)}%` }}></div>
        </div> 
    </div> 
);
...
const ResultsSkeleton = () => ( 
    <div className="space-y-8 animate-pulse"> 
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8"> 
            <div className="lg:col-span-1 bg-[#1C1C1C] p-6 rounded-lg flex flex-col items-center justify-center border border-gray-800"> 
            ...
            <div className="lg:col-span-2 bg-[#1C1C1C] p-6 rounded-lg border border-gray-800"> 
        ...
        <div className="bg-[#1C1C1C] rounded-lg border border-gray-800"> 
            <div className="p-6"> <SkeletonLoader className="h-8 w-1/4 rounded-md" /> </div> 
            <div className="space-y-4 p-6 border-t border-gray-800"> 
        ...
        <div className="lg:col-span-1 bg-[#1C1C1C] p-6 rounded-lg flex flex-col items-center justify-center border border-gray-800">
            ...
            <div className="relative w-40 h-40">
                <svg className="w-full h-full" viewBox="0 0 36 36">
                    ...
                    <path className="text-teal-400" strokeDasharray={`${results.overall_score}, 100`} d="M18 2.0845 a 15.9155 15.9155 0 0 1 0 31.831 a 15.9155 15.9155 0 0 1 0 -31.831" fill="none" stroke="currentColor" strokeWidth="3" strokeLinecap="round"></path>
                </svg>
      ...
      <div className="bg-[#1C1C1C] rounded-lg border border-gray-800">
        ...
        <div className="space-y-4 p-6 border-t border-gray-800">
            {results.questions.map((q, index) => (
                <details key={q.id} className="bg-black/20 rounded-lg p-4 border border-gray-800" open={index === 0}>
                    <summary className="font-semibold text-lg cursor-pointer text-gray-200 hover:text-white">Q: {q.question_text}</summary>
                    <div className="mt-4 grid grid-cols-1 md:grid-cols-2 gap-6 pt-4 border-t border-gray-700">
                        <div>
                            <h4 className="font-bold text-teal-400 mb-2 flex items-center gap-2"><CheckIcon /> Strengths</h4>
                            <ul className="space-y-2 pl-2 text-sm list-disc list-inside text-gray-300">
                                {(q.ai_feedback?.strengths || ['No specific strengths identified.']).map((s, i) => (<li key={i}>{s}</li>))}
                            </ul>
                        </div>
                        <div>
                            <h4 className="font-bold text-yellow-400 mb-2 flex items-center gap-2"><XIcon /> Areas for Improvement</h4>
                            <ul className="space-y-2 pl-2 text-sm list-disc list-inside text-gray-300">
                                {(q.ai_feedback?.improvements || ['No specific improvements identified.']).map((imp, i) => (<li key={i}>{imp}</li>))}
                            </ul>
                        </div>
                    </div>
                </details>
