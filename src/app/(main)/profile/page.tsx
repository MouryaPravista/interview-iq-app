...
const ActivityCalendar = ({ data }: { data: ActivityData }) => {
    ...
    const squares = useMemo(() => {
        const result = [];
        // Create a grid of squares for the last ~6 months
        for (let i = 181; i >= 0; i--) {
...
// --- THE MAIN (AND ONLY) DEFAULT EXPORT FOR THIS PAGE ---
export default function ProfilePage() {
    ...
    return (
        <div className="max-w-6xl mx-auto space-y-12">
            <div className="flex items-center gap-6">
                <div className="h-20 w-20 rounded-full bg-blue-600 flex items-center justify-center font-bold text-3xl flex-shrink-0 border-2 border-blue-400">{getInitials()}</div>
            ...
            <div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    {stats ? (
                        ...
                        <div className="md:col-span-3 text-center text-gray-500 bg-gray-800 p-6 rounded-lg border border-gray-700">Complete an interview to see your stats!</div>
                    )}
                </div>
            </div>
                    ...
                    <div className="space-y-4">
                        <div>
                            ...
                            {isEditingName ? (
                                ...
                                <div className="flex items-center justify-between"><p className="text-lg">{fullName || 'Not set'}</p><button onClick={() => setIsEditingName(true)} className="text-sm text-blue-400 hover:underline">Edit</button></div>
                            )}
                        </div>
